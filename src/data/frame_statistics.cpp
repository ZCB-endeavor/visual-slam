...
#include "vslam/data/frame_statistics.h"
    ...
    namespace data {
        void frame_statistics::update_frame_statistics(const data::frame &frm, const bool is_lost) {
            if (frm.cam_pose_cw_is_valid_) {
                ...
                assert(!img_name_.count(frm.id_));
                img_name_[frm.id_] = frm.image_name_;
            }
        ...
        void frame_statistics::replace_reference_keyframe(data::keyframe *old_keyfrm, data::keyframe *new_keyfrm) {
            ...
            assert(num_valid_frms_ == img_name_.size());
        ...
        std::unordered_map<data::keyframe *, std::vector<unsigned int>>
        ...
        std::map<unsigned int, std::string> frame_statistics::get_image_name() const {
            return {img_name_.begin(), img_name_.end()};
        }
