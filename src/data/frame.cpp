...
namespace vslam {
    namespace data {
        ...
        frame::frame(const std::string image_name, const cv::Mat &img_gray, const double timestamp,
                     feature::orb_extractor *extractor, bow_vocabulary *bow_vocab,
                     camera::base *camera, const float depth_thr,
                     const cv::Mat &mask)
                : image_name_(image_name), id_(next_id_++), bow_vocab_(bow_vocab), extractor_(extractor),
        ...
        frame::get_keypoints_in_cell(const float ref_x, const float ref_y, const float margin, const int min_level,
