...
namespace vslam {
    ...
    void mapping_module::create_new_landmarks() {
        ...
        const auto cur_covisibilities = cur_keyfrm_->graph_node_->get_top_n_covisibilities(
                num_covisibilities * heuristic_ratio);
        ...
        for (unsigned int i = 0; i < cur_covisibilities.size(); ++i) {
            ...
            const Mat33_t E_ngh_to_cur = solve::essential_solver::create_E_21(ngh_keyfrm->get_rotation(),
                                                                              ngh_keyfrm->get_translation(),
                                                                              cur_keyfrm_->get_rotation(),
...
#pragma omp critical
    ...
    mapping_module::get_second_order_covisibilities(const unsigned int first_order_thr,
