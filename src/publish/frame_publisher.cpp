...
namespace vslam {
    namespace publish {
        ...
        void frame_publisher::draw_info_text(cv::Mat &img, const tracker_state_t tracking_state,
            ...
            img.copyTo(overlay);
            cv::rectangle(overlay, cv::Point2i{0, img.rows - text_size.height - 10}, cv::Point2i{img.cols, img.rows},
                          cv::Scalar{0, 0, 0}, -1);
            cv::addWeighted(overlay, alpha, img, 1 - alpha, 0, img);
            // put text
            cv::putText(img, ss.str(), cv::Point(5, img.rows - 5), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar{255, 255, 255},
                        1, 8);
        }
