...
#include "vslam/module/two_view_triangulator.h"
#include "vslam/solve/triangulator.h"
    ...
    namespace module {
        two_view_triangulator::two_view_triangulator(data::keyframe *keyfrm_1, data::keyframe *keyfrm_2,
                ...
                : keyfrm_1_(keyfrm_1), keyfrm_2_(keyfrm_2),
                  rot_1w_(keyfrm_1->get_rotation()), rot_w1_(rot_1w_.transpose()),
                  trans_1w_(keyfrm_1->get_translation()),
                  cam_pose_1w_(keyfrm_1->get_cam_pose()), cam_center_1_(keyfrm_1->get_cam_center()),
                  camera_1_(keyfrm_1->camera_),
                  rot_2w_(keyfrm_2->get_rotation()), rot_w2_(rot_2w_.transpose()),
                  trans_2w_(keyfrm_2->get_translation()),
                  cam_pose_2w_(keyfrm_2->get_cam_pose()), cam_center_2_(keyfrm_2->get_cam_center()),
        ...
        bool two_view_triangulator::triangulate(const unsigned idx_1, const unsigned int idx_2, Vec3_t &pos_w) const {
            ...
            const auto cos_stereo_parallax_1 = is_stereo_1
                                               ? std::cos(
                            2.0 * atan2(camera_1_->true_baseline_ / 2.0, keyfrm_1_->depths_.at(idx_1)))
                                               : 2.0;
            const auto cos_stereo_parallax_2 = is_stereo_2
                                               ? std::cos(
                            2.0 * atan2(camera_2_->true_baseline_ / 2.0, keyfrm_2_->depths_.at(idx_2)))
                                               : 2.0;
        ...
        template<typename T>
        bool two_view_triangulator::check_reprojection_error(const Vec3_t &pos_w, const Mat33_t &rot_cw,
                                                             const Vec3_t &trans_cw, camera::base *const camera,
