...
#include "vslam/initialize/bearing_vector.h"
#include "vslam/initialize/perspective.h"
#include "vslam/match/area.h"
#include "vslam/module/initializer.h"
#include "vslam/optimize/global_bundle_adjuster.h"
...
namespace vslam {
    namespace module {
        ...
        bool initializer::try_initialize_for_monocular(data::frame &curr_frm) {
            ...
            const auto num_matches = matcher.match_in_consistent_area(init_frm_, curr_frm, prev_matched_coords_,
                                                                      init_matches_, 100);
            ...
            assert(initializer_);
            spdlog::debug("try to initialize with the initial frame and the current frame: frame {} - frame {}",
        ...
        bool initializer::create_map_for_monocular(data::frame &curr_frm) {
            ...
            const auto median_depth = init_keyfrm->compute_median_depth(false);
            ...
            spdlog::info("new map created with {} points: frame {} - frame {}", map_db_->get_num_landmarks(),
                         init_frm_.id_, curr_frm.id_);
