...
#include "vslam/data/bow_database.h"
#include "vslam/match/bow_tree.h"
#include "vslam/match/projection.h"
#include "vslam/module/loop_detector.h"
#include "vslam/solve/sim3_solver.h"
#include "vslam/util/converter.h"
...
namespace vslam {
    namespace module {
        loop_detector::loop_detector(data::bow_database *bow_db, data::bow_vocabulary *bow_vocab,
                                     const YAML::Node &yaml_node, const bool fix_scale_in_Sim3_estimation)
        ...
        bool loop_detector::detect_loop_candidates() {
            ...
            const auto curr_cont_detected_keyfrm_sets = find_continuously_detected_keyframe_sets(
                    cont_detected_keyfrm_sets_, init_loop_candidates);
        ...
        bool loop_detector::validate_candidates() {
            ...
            const bool candidate_is_found = select_loop_candidate_via_Sim3(loop_candidates_to_validate_,
                                                                           selected_candidate_,
                                                                           g2o_Sim3_world_to_curr_,
            ...
            spdlog::debug("detect loop candidate via Sim3 estimation: keyframe {} - keyframe {}",
            ...
            if (num_final_matches_thr_ <= num_final_matches) {
                ...
                spdlog::debug("destruct loop candidate because enough matches not acquired (< {})",
        ...
        keyframe_sets
        loop_detector::find_continuously_detected_keyframe_sets(const keyframe_sets &prev_cont_detected_keyfrm_sets,
        ...
        bool loop_detector::select_loop_candidate_via_Sim3(const std::vector<data::keyframe *> &loop_candidates,
            ...
            for (const auto candidate : loop_candidates) {
                ...
                const auto num_matches = bow_matcher.match_keyframes(cur_keyfrm_, candidate,
                ...
                spdlog::debug("found loop candidate via linear Sim3 estimation: keyframe {} - keyframe {}",
                ...
                projection_matcher.match_keyframes_mutually(cur_keyfrm_, candidate, curr_match_lms_observed_in_cand,
                                                            scale_cand_to_curr, rot_cand_to_curr, trans_cand_to_curr,
                                                            7.5);

                g2o::Sim3 g2o_sim3_cand_to_curr(rot_cand_to_curr, trans_cand_to_curr, scale_cand_to_curr);
                const auto num_optimized_inliers = transform_optimizer_.optimize(cur_keyfrm_, candidate,
                                                                                 curr_match_lms_observed_in_cand,
                ...
                spdlog::debug("found loop candidate via nonlinear Sim3 optimization: keyframe {} - keyframe {}",
                              candidate->id_, cur_keyfrm_->id_);
