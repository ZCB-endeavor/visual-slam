...
namespace vslam {
    global_optimization_module::global_optimization_module(data::map_database *map_db, data::bow_database *bow_db,
            ...
            : loop_detector_(
            new module::loop_detector(bow_db, bow_vocab, util::yaml_optional_ref(yaml_node, "LoopDetector"),
                                      fix_scale)),
    ...
    void global_optimization_module::correct_loop() {
        ...
        graph_optimizer_->optimize(final_candidate_keyfrm, cur_keyfrm_, Sim3s_nw_before_correction,
                                   Sim3s_nw_after_correction, new_connections);
        ...
        thread_for_loop_BA_ = std::unique_ptr<std::thread>(
                new std::thread(&module::loop_bundle_adjuster::optimize, loop_bundle_adjuster_.get(),
                                cur_keyfrm_->id_));
    ...
    global_optimization_module::get_Sim3s_before_loop_correction(const std::vector<data::keyframe *> &neighbors) const {
    ...
    module::keyframe_Sim3_pairs_t
    global_optimization_module::get_Sim3s_after_loop_correction(const Mat44_t &cam_pose_wc_before_correction,
