...
#include "vslam/module/local_map_updater.h"
#include "vslam/util/image_converter.h"
...
namespace vslam {
    tracking_module::tracking_module(const std::shared_ptr<config> &cfg, system *system, data::map_database *map_db,
            ...
            : camera_(cfg->camera_), true_depth_thr_(get_true_depth_thr(camera_, cfg->yaml_node_)),
              ...
              initializer_(cfg->camera_->setup_type_, map_db, bow_db,
    ...
    tracking_module::track_monocular_image(const std::string image_name, const cv::Mat &img, const double timestamp,
                                           const cv::Mat &mask) {
        ...
        if (tracking_state_ == tracker_state_t::NotInitialized || tracking_state_ == tracker_state_t::Initializing) {
            curr_frm_ = data::frame(image_name, img_gray_, timestamp, ini_extractor_left_, bow_vocab_, camera_,
                                    true_depth_thr_, mask);
        } else {
            curr_frm_ = data::frame(image_name, img_gray_, timestamp, extractor_left_, bow_vocab_, camera_,
                                    true_depth_thr_, mask);
        }
        ...
        if (curr_frm_.cam_pose_cw_is_valid_) {
            cam_pose_wc = std::allocate_shared<Mat44_t>(Eigen::aligned_allocator<Mat44_t>(),
                                                        curr_frm_.get_cam_pose_inv());
        }
        return cam_pose_wc;
    }
