...
#include "vslam/util/stereo_rectifier.h"
...
namespace vslam {
    namespace util {
        stereo_rectifier::stereo_rectifier(const std::shared_ptr<vslam::config> &cfg)
        ...
        stereo_rectifier::stereo_rectifier(camera::base *camera, const YAML::Node &yaml_node)
            ...
            switch (model_type_) {
                case camera::model_type_t::Perspective: {
                    cv::initUndistortRectifyMap(K_l, D_l, R_l, K_rect, img_size, CV_32F, undist_map_x_l_,
                                                undist_map_y_l_);
                    cv::initUndistortRectifyMap(K_r, D_r, R_r, K_rect, img_size, CV_32F, undist_map_x_r_,
                                                undist_map_y_r_);
                    break;
                }
                default: {
