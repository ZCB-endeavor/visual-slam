...
#include "vslam/tracking_module.h"
#include "vslam/mapping_module.h"
#include "vslam/global_optimization_module.h"
#include "vslam/camera/base.h"
#include "vslam/data/camera_database.h"
#include "vslam/data/map_database.h"
#include "vslam/data/bow_database.h"
#include "vslam/data/bow_vocabulary.h"
#include "vslam/io/trajectory_io.h"
#include "vslam/io/map_database_io.h"
#include "vslam/publish/map_publisher.h"
#include "vslam/publish/frame_publisher.h"
...
namespace vslam {
    ...
    void system::startup(const bool need_initialize) {
        ...
        mapping_thread_ = std::unique_ptr<std::thread>(new std::thread(&vslam::mapping_module::run, mapper_));
        global_optimization_thread_ = std::unique_ptr<std::thread>(
                new std::thread(&vslam::global_optimization_module::run, global_optimizer_));
    }
...
//    }
    ...
    system::feed_monocular_frame(const std::string image_name, const cv::Mat &img, const double timestamp,
                                 const cv::Mat &mask) {
        ...
        const auto cam_pose_wc = tracker_->track_monocular_image(image_name, img, timestamp, mask);
        const auto tp_2 = std::chrono::steady_clock::now();
        const auto track_time = std::chrono::duration_cast<std::chrono::duration<double>>(tp_2 - tp_1).count();
        // std::cout << "track_monocular_image 跟踪时间: " << track_time << std::endl;
