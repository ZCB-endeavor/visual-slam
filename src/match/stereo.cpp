...
#pragma omp parallel for
            ...
            for (unsigned int idx_left = 0; idx_left < num_keypts_; ++idx_left) {
                ...
                const auto is_valid = compute_subpixel_disparity(keypt_left, keypt_right, best_x_right, best_disp,
                                                                 best_correlation);
...
#pragma omp critical
        ...
        void stereo::find_closest_keypoints_in_stereo(const unsigned int idx_left, const int scale_level_left,
                                                      ...
                                                      unsigned int &best_idx_right,
                                                      unsigned int &best_hamm_dist) const {
            best_idx_right = 0;
        ...
        bool stereo::compute_subpixel_disparity(const cv::KeyPoint &keypt_left, const cv::KeyPoint &keypt_right,
            ...
            patch_left -=
                    patch_left.at<float>(win_size, win_size) * cv::Mat::ones(patch_left.rows, patch_left.cols, CV_32F);
