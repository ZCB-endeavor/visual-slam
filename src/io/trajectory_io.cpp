...
namespace vslam {
    namespace io {
        ...
        void trajectory_io::save_frame_trajectory(const std::string &path, const std::string &format) const {
            ...
            const auto image_name = frm_stats.get_image_name();
            ...
            spdlog::info("dump frame trajectory in \"{}\" format from frame {} to frame {} ({} frames)",
                         format, reference_keyframes.begin()->first, reference_keyframes.rbegin()->first,
                         num_valid_frms);
            ...
            for (unsigned int i = 0; i < num_valid_frms; ++i, ++rk_itr, ++rc_itr) {
                ...
                std::cout << "frame_id:  " << frm_id << std::endl;
                std::cout << "image_name:  " << image_name.at(frm_id) << std::endl;
                ...
                if (format == "KITTI") {
                    ofs << std::setprecision(9)
                        << cam_pose_wc(0, 0) << " " << cam_pose_wc(0, 1) << " " << cam_pose_wc(0, 2) << " "
                        << cam_pose_wc(0, 3) << " "
                        << cam_pose_wc(1, 0) << " " << cam_pose_wc(1, 1) << " " << cam_pose_wc(1, 2) << " "
                        << cam_pose_wc(1, 3) << " "
