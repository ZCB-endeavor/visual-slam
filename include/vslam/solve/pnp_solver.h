...
#ifndef VSLAM_SOLVE_PNP_SOLVER_H
#define VSLAM_SOLVE_PNP_SOLVER_H
...
namespace vslam {
    namespace solve {
        ...
        private:
            ...
            double reprojection_error(const eigen_alloc_vector<Vec3_t> &pws, const eigen_alloc_vector<Vec3_t> &bearings,
            ...
            eigen_alloc_vector<Vec4_t> compute_barycentric_coordinates(const eigen_alloc_vector<Vec3_t> &control_points,
                                                                       const eigen_alloc_vector<Vec3_t> &pos_ws);
            ...
            eigen_alloc_vector<Vec3_t>
            compute_pcs(const eigen_alloc_vector<Vec4_t> &alphas, const eigen_alloc_vector<Vec3_t> &ccs,
                        const bool bearing_z_sign);
            ...
            void compute_A_and_b_for_gauss_newton(const MatRC_t<6, 10> &L_6x10, const Vec6_t &Rho, const Vec4_t &betas,
                                                  MatRC_t<6, 4> &A, Vec6_t &b);

            //! Estimate R and t by the local 3D points and the world 3D points
            void
            estimate_R_and_t(const eigen_alloc_vector<Vec3_t> &pws, const eigen_alloc_vector<Vec3_t> &pcs, Mat33_t &rot,
                             Vec3_t &trans);
...
#endif //VSLAM_SOLVE_PNP_SOLVER_H
